...
public interface PedidoRepository extends JpaRepository<Pedido, Long> {
    ...
    // 2. COALESCE garante que se o banco estiver vazio, retornará 0 em vez de null
    @Query("SELECT COALESCE(SUM(p.valorTotal), 0) FROM Pedido p WHERE p.dataEncomenda BETWEEN :inicio AND :fim")
    ...
    // 4. Busca os produtos mais vendidos por quantidade (Corrigido: removido o campo 'sabor' inexistente)
